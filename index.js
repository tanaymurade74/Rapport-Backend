...
async function updateLeadById(id, lead){
    try{
        const updatedLead = await Lead.findByIdAndUpdate({_id: id}, lead, {new: true})
        return updatedLead;
    }catch(error){
        throw error;
    }
...
app.get("/report/last-week", async(req, res)=>{
    try{
        ...
        if(leadsClosed.length === 0){
            return res.status(404).json({Error:"No leads were closed in the past week"})
        }
        return res.status(200).json(leadsClosed.map(lead => ({id: lead._id, name: lead.name,
             salesAgent: lead.salesAgent, closedAt: lead.closedAt })))
    }catch{
...
app.get("/report/pipeline", async(req, res)=>{
    try{
        ...
        return res.status(500).json({Error: "Error occurred while trying to fetch reports"})
    }
})
...
app.get("/agents", async (req, res)=>{
    try{
        ...
        return res.status(500).json({Error: "Error occurred while trying to get all salesAgents"})
    }
})
...
app.post("/agents", async(req, res)=>{
    try{
        ...
        return res.status(500).json({Error: "Error occurred while trying to create salesAgent"})
    }
})
...
app.get("/leads", async(req, res) => {
    try{
        ...
        if (status && !allowedStatuses.includes(status)) {
             return res.status(400).json({ error: "Invalid input: 'status' must be one of the allowed values." });
        }
        ...
        return res.status(500).json({Error: "Error while trying to get all leads."})
    }
})

app.post("/leads", async (req, res)=>{
    try{
        const { salesAgent, name, source } = req.body;

        if (!name || !source) {
            return res.status(400).json({ error: "Invalid input: 'name' and 'source' are required." });
        }
        ...
        return res.status(500).json({Error: "Error occurred while trying to create new lead"})
    }
})
...
app.delete("/leads/:id", async(req, res)=>{
    try{
        ...
        return res.status(500).json({Error: "Error occurred while trying to delete"})
    }
})
...
app.put("/leads/:id", async(req, res)=> {
    try{
        ...
        return res.status(500).json({Error: "Error occurred while trying to update lead"})
    }
})
...
app.post("/leads/:id/comments", async (req, res) => {
    try {
        ...
        const savedComment = await addCommentToLead(leadId, { commentText, author });
        
        return res.status(201).json(savedComment);
    } catch (error) {
        console.error(error);
        return res.status(500).json({ Error: "Error occurred while adding comment." });
    }
});
...
app.get("/leads/:id/comments", async (req, res)=>{
    try{
        ...
        return res.status(500).json({Error: "Error occurred while trying to fetch all comments for the lead"})
    }
})
...
app.post("/tag", async (req, res)=>{
    try{
        ...
        return res.status(500).json({Error: "An error occurred while trying to create tag"})
    }
})
...
app.get("/tag", async(req, res)=>{
    try{
        ...
        return res.status(500).json({Error: "An error occurred while trying to get all tags"})
    }
})
